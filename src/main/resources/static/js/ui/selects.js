...
/**
 * Fills a <select> element with <option>s built from a list of items.
 *
 * @param {HTMLSelectElement} sel - the select element to populate
 * @param {Array} items - strings or objects to turn into options
 * @param {Object} [opts]
 * @param {string} [opts.placeholder] - disabled/hidden placeholder option text
 * @param {boolean} [opts.sort=true] - alphabetically sort options by label
 * @param {(item) => {label: string, value: string}} [opts.optionFor] -
 *   custom label/value mapper; defaults to guessName() for objects
 */
